...
export default function SignupScreen() {
  ...
  const submitHandler = async (e) => {
    ...
    try {
      const usernameWithoutSpaces = name.replace(/\s/g, '');
      ...
      if (data.message === 'Signup successful. Please check your email for verification') {
  ...
  return (
    <Container className="small-container">
      ...
      <Form onSubmit={submitHandler}>
        ...
        <div className="mb-3">
          Sign up with google{' '}
          <Link to='#' onClick={handleSignInWithGoogle}>Sign-Up</Link>
        </div>
      </Form>
    </Container>
  );
